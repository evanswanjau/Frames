...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    {
        return view('provider.passwords.email');
    }
    public function sendResetLinkEmail(Request $request)
    {
        $this->validateEmail($request);

        $us = Provider::whereEmail($request->email)->count();
        if ($us == 0)
        {
            ...
            $user1 = Provider::whereEmail($request->email)->first();
            $route = 'provider.password.reset';
