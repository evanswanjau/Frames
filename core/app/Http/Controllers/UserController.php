...
class UserController extends Controller
{
    ...
    public function getCheckOut()
    {
        ...
        if (count($cart) == 0){
            ...
            $data['country'] = json_decode(file_get_contents('http://country.io/names.json'),true);
            $data['userDetails'] = UserDetails::whereUser_id(Auth::user()->id)->first();
            $data['routes'] = \DB::select('select * from routes order by town');
    ...
    public function submitDetails(Request $request)
    {
        ...
        $request->validate([
           ...
           's_number' => 'required|numeric', //'s_company' => 'required',
           's_country' => 'required',
           's_city' => 'required',
           's_address' => 'required',
        ]);
    ...
    public function orderOverview()
    {
        ...
        $routes = \DB::select('select * from routes where id =' . $data['userDetails']->s_city);

        $data['shipping'] = $routes[0]->total;
    ...
    public function orderSubmit(Request $request)
    {
        ...
        if (Cart::count() == 0){
            ...
            $userDetails = UserDetails::whereUser_id(Auth::user()->id)->first();
            $message['phone_number'] = $userDetails->s_number;
            $message['message'] = "Dear " . Auth::user()->first_name . ",\nYour order " . $order['order_number'] . " has been confirmed! Please check your email " . $userDetails->s_email . " for the complete order details.";

            // send SMS
            //app(\App\Http\Controllers\BulkSmsController::class)->sendMessage($message);
            ...
            if ($request->payment_id == 1) {
                return view('home.pesaPal');
            }elseif($request->payment_id == 2){
                return Redirect::to('https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=BNE9HR589FUYQ');
            }
    ...
    public static function paymentComplete($userId, $orderId){
        ...
        $data['logoUrl'] = asset('assets/images/logo.png');
        $data['invoiceNumber'] = $order->order_number;
        $data['invoiceDate'] = Carbon::parse($order->created_at)->format('F d, Y - h:i A');
        $data['companyDetails'] = $basic->title."<br>".$basic->phone."<br>".$basic->address;
        $data['userDetails'] = $userDetails->b_name."<br>".$userDetails->b_number."<br>".$userDetails->b_email;
        $data['paymenrUrl'] = route('payment',$order->order_number);
        $data['subtotal'] = $basic->symbol.$order->subtotal;
        $data['total'] = $basic->symbol.$order->total;
        $data['totalTax'] = $basic->symbol.$order->tax;
        $data['tax'] = $basic->tax;
        $data['minute'] = $basic->expire_time;

        $data['items'] = OrderItem::whereOrder_id($orderId)->get();

        for ($i=0; $i < count($data['items']); $i++) { 

            $product_id = $data['items'][$i]['product_id'];
            $product = Product::whereid($product_id)->get();
            $data['items'][$i]['name'] = $product[0]["name"];
            $data['items'][$i]['rate'] = $product[0]["current_price"];
            $data['items'][$i]['total'] = $product[0]["current_price"] * $data['items'][$i]['qty'];

        }

        return view('home.payment-complete', $data);
